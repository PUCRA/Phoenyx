...
class FSM(Node):
    ...
    def FSM(self):
        ...
        #S3: Navegar a travès de los waypoints
            ...
            if self.first:
                self.get_logger().info("Estado 3: usando FollowWaypoints para toda la lista")
                self.first = False

                # Construimos la lista de PoseStamped
                ...
                for wp in self.waypoints:
                    ...
                    pose.pose.orientation.w = 1.0  # quaternion neutro
                    goal.poses.append(pose)

                # Enviamos el goal
                send_goal_future = self.wp_client.send_goal_async(
                    goal,
                    feedback_callback=self.wp_feedback_callback
                )
                send_goal_future.add_done_callback(self.wp_response_callback)
            else:
                self.get_logger().info("Todos los waypoints alcanzados.")
                self.state = 4
             
                
        # #S4: estado navegacion
        # elif self.state == 4:
        #     self.get_logger().info("Estado 4: Navegando")
        #     if self._get_result_future and self._get_result_future.done():
        #         status = self._get_result_future.result().status
        #         if status == 4: #4 significa goal reached
                    # self.goal_reached = True
    # opcional: procesar otros códigos (canceled, aborted…)

            # if total_wp == self.waypoint_index and self.goal_reached:
                # self.state = 5
    ...
    def wp_response_callback(self, future):
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().warn('FollowWaypoints rechazado')
            return
        self.get_logger().info('FollowWaypoints aceptado, esperando resultado…')
        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self.wp_result_callback)

    def wp_feedback_callback(self, feedback_msg):
        dist = feedback_msg.feedback.current_waypoint
        self.get_logger().info(f'Feedback waypoint actual: {dist}')

    def wp_result_callback(self, future):
        result = future.result().result
        self.get_logger().info(f'Seguimiento completado con estado {future.result().status}')
        # Aquí puedes avanzar al estado 4 o lo que necesites
        self.state = 4
